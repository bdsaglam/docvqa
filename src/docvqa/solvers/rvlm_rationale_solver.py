"""RVLM rationale variant — the VLM returns a clean answer PLUS a short observation note.

Fork of :mod:`docvqa.solvers.rvlm_solver` (the proposed method). In `rvlm`
the perception sub-call (`batch_look`) returns ONLY a terse answer ("2048.88",
"Unknown"). Here the VLM additionally returns a brief **observation/uncertainty
note** alongside the answer, on EVERY call — what is visible and legible in the
region, and any doubt (small/blurry text, a digit that could be misread, the
region clipped at the crop edge, the value not present on this page).

The `answer` field stays clean and terse (so the main agent still parses it
directly); the note is a SEPARATE structured field, surfaced to the agent as
`<answer>  [note: <observation>]`. The hypothesis: the note gives the main
agent diagnostic signal to steer its next action (re-crop wider, try another
page, trust vs re-verify) that a bare answer cannot.

**Framing (deliberate):** the note is an *observation*, not a *justification* —
the prompt tells the VLM to surface uncertainty rather than inflate confidence,
so it does not undermine `rvlm`'s "the VLM is unreliable, verify everything"
discipline (which is load-bearing). Only the VLM signature, the `batch_look`
return format, and a one-paragraph prompt nudge change vs `rvlm`.
"""
...
# ---------------------------------------------------------------------------
# VLM sub-call: concise answer + a short observation/uncertainty note.
# ---------------------------------------------------------------------------

def _build_vlm_predict() -> dspy.Predict:
    return dspy.Predict(
        dspy.Signature(
            {
                ...
                "answer": (str, dspy.OutputField(desc="The concise answer ONLY (a value/word/list). Exact transcription; 'Unknown' if missing.")),
                "rationale": (str, dspy.OutputField(desc="ONE brief observation note: what is visible/legible in the relevant region and any uncertainty.")),
            },
            "Analyze the image strictly to answer the query.\n"
            "- `answer`: the concise answer ONLY — transcribe numbers and characters exactly, keep it as "
            "short as the query allows (a value, a word, a list). If the information is missing, output 'Unknown'. "
            "When a label is separated from the item it identifies, trace any visual connector (leader line, "
            "arrow, callout, alignment) to determine which item it refers to.\n"
            "- `rationale`: ONE brief sentence of OBSERVATION supporting the answer — what is actually visible "
            "and legible in the relevant region, and CRUCIALLY any uncertainty: small or blurry text, a digit/letter "
            "that could be misread, the region appearing cut off at the image edge, or the value not being present "
            "on this image. This is an observation note for a downstream agent, NOT a justification — do not inflate "
            "confidence; surface doubt when it exists (e.g. 'the third digit is blurry, could be 3 or 8'; 'a revenue "
            "table is here but its right column is clipped'; 'no total row visible on this page').",
        )
    )
...
def _create_tools(vlm_predict: dspy.Predict, vlm_lm: dspy.LM, batch_concurrency: int = 8) -> list:
    ...
    def _look_impl(image_path: str, query: str) -> str:
        """Load image, send to VLM, return 'answer  [note: rationale]'."""
        with logfire.span("look", image_path=image_path, query=query) as span:
            ...
            note = str(getattr(result, "rationale", "") or "").strip()
            span.set_attribute("answer", answer[:2000])
            span.set_attribute("rationale", note[:2000])
            if note:
                # one line; collapse internal newlines so the agent's parse stays simple
                note_1l = " ".join(note.split())
                return f"{answer}  [note: {note_1l}]"
            return answer
    ...
    def _batch_look_impl(requests_json: str) -> list[str]:
        ...
        with logfire.span("batch_look", num_requests=len(requests)):
            with ThreadPoolExecutor(max_workers=max_w) as pool:
                futures = {pool.submit(_do, i, r["path"], r["query"]): i for i, r in enumerate(requests)}
...
def _build_sandbox_code(page_dir: str, num_pages: int) -> str:
    """Sandbox: load pages, define `batch_look` (each result = answer + [note: ...])."""
    return f'''
import os
import tempfile
...
def batch_look(requests):
    """Send multiple images to the VLM in parallel. Input: list of (image, query) tuples.
    Returns: list of str, same order. Each result is the concise answer FOLLOWED BY a brief
    `[note: ...]` observation — what was visible/legible and any uncertainty (blurry digits,
    a clipped region, the value not on this image). Use the note to decide whether to trust
    the read or re-look (wider crop, different page). Example:
      batch_look([(pages[0], "read the total revenue")])  ->  ["2048.88  [note: crisp, top of TSR table]"]"""
    import json as _json
    paths = []
...
# ---------------------------------------------------------------------------
# Task body: rvlm's, with a one-paragraph nudge about the [note: ...] channel.
# ---------------------------------------------------------------------------
...
_TASK_BODY = (
    ...
    "  Each result is the concise answer FOLLOWED BY a brief `[note: ...]` — an observation of "
    "what was visible/legible and any uncertainty the VLM flagged (blurry or ambiguous "
    "characters, a region clipped at the crop edge, the value not present on the image). "
    "USE the note: if it flags uncertainty or a clipped/absent region, do not commit the "
    "reading — re-look with a wider or shifted crop, a different page, or a rephrased query.\n"
    "- SUBMIT(answer=\"...\")\n"
    ...
    "do not commit a reading you've only seen once. Let the `[note: ...]` guide "
    "you: a note flagging blur/ambiguity/clipping is a signal to re-read with a "
    "different crop or query; look for consistency across reads, or cross-"
    "reference an adjacent label. See the verification guidance below.\n"
    ...
    "single read is not trustworthy, and a confident-sounding `[note: ...]` "
    "is not proof — treat the note as a hint about where to look harder, not "
    "as a guarantee. Build a reading procedure that compensates: read the same "
    "region multiple times and look for the consistent answer; read at multiple "
    "crop sizes or framings; rephrase the query; tile-scan a region too large "
    "for one read; cross-check against an adjacent label or value. Be aware of "
...
class RvlmRationaleProgram:
    """RVLM with answer+observation-note perception sub-call. See module docstring."""
    ...
    def __init__(
        ...
        self.vlm_predict = _build_vlm_predict()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def _solve_question(q: Question):
                with logfire.span(
                    ...
                    logger.info(
                        "RVLM-RATIONALE [%s] (%s) Q %s: max_iterations=%d (page_bonus=%d)",
                    ...
                    if q.answer is not None:
                        ...
                        logger.info(
                            "RVLM-RATIONALE[%s] Q %s: %s (GT=%s, PRED=%s)",
                            self.profile.name, q.question_id,
                            "CORRECT" if is_correct else "WRONG", q.answer[:40], extracted[:40],
                        )
            ...
            if self.question_concurrency <= 1:
                ...
                logger.info("RVLM-RATIONALE: running %d questions with concurrency=%d", len(document.questions), max_w)
                with ThreadPoolExecutor(max_workers=max_w) as pool:
            ...
            for q in document.questions:
                if q.answer is not None:
                    ...
                    if self.profile.score_fn(predictions[q.question_id], q.answer, q)[0]:
                        correct += 1
            if scored > 0:
                logger.info("RVLM-RATIONALE [%s] doc %s: %d/%d = %.1f%%",
                            self.profile.name, document.doc_id, correct, scored, 100 * correct / scored)
            return predictions, trajectories


def create_rvlm_rationale_program(
    ...
    question_concurrency: int = 4,
    batch_concurrency: int = 8,
) -> RvlmRationaleProgram:
    """Hydra factory. Mirrors create_rvlm_program; only the VLM sub-call adds a rationale note."""
