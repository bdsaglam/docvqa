"""Dataset-aware leanest_solo (no OCR, only batch_look).

A clone of :mod:`docvqa.solvers.leanest_solo_solver` with a
:class:`docvqa.datasets.profile.DatasetProfile` injected via the
constructor. The profile drives the answer-formatting rules, the
per-category tips, the per-question format hint, and (for the runner)
the scoring function — same as ``flat_solo_da_solver``.

Differs from ``flat_solo_da`` in what tooling the agent has access to:
``leanest_solo`` exposes only ``batch_look`` (no OCR text, no BM25
search), so the agent's only channel into the document is active VLM
perception via page/crop calls.

Helpers (``_create_tools``, ``_build_sandbox_code``, ``_build_signature``)
are imported from :mod:`docvqa.solvers.leanest_solo_solver` — those
are tooling implementations that don't need to change per dataset.
"""
...
from docvqa.solvers.leanest_solo_solver import (
    _build_sandbox_code,
    _build_signature,
    _create_tools,
)
...
# ---------------------------------------------------------------------------
# LeanestSoloDAProgram
# ---------------------------------------------------------------------------


class LeanestSoloDAProgram:
    """Dataset-aware leanest_solo. See module docstring."""
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def _solve_question(q: Question):
                with logfire.span(
                    ...
                    logger.info(
                        "Leanest solo DA [%s] (%s) Q %s: max_iterations=%d (page_bonus=%d)",
            ...
            if self.question_concurrency <= 1:
                ...
                logger.info("Leanest solo DA: running %d questions with concurrency=%d", len(document.questions), max_w)
            ...
            if scored > 0:
                logger.info(
                    "Leanest solo DA [%s] doc %s: %d/%d = %.1f%%",
...
def create_leanest_solo_da_program(
