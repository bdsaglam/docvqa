"""Ablation variant of ``rvlm_solver``: unified category tips.

Identical to ``RvlmProgram`` (code-LM in REPL with recursive VLM sub-call
via ``batch_look``) EXCEPT it ignores ``document.doc_category`` and
injects the concatenation of ALL 8 category tip blocks instead of
dispatching per-document.

Purpose. Tests whether the scaffold requires per-document category
metadata or whether a unified prompt suffices. Defends against the
reviewer objection "this only works because you hand it the category as
metadata."

Predicted outcomes:
- Δ ≈ 0pp (within noise): per-category dispatch is procedural
  convenience, not load-bearing. Strongest paper outcome — promote
  unified to default.
- Δ −1 to −3pp: small prompt-length cost; still paper-strengthening
  cell ("our method does not require category labels").
- Δ −5pp or worse: category dispatch IS load-bearing; honest caveat in
  paper.

See ``docs/experiments/unified-category-tips-ablation.md`` for the full
hypothesis and expected experimental design.

Implementation. Pulls ``_DOCVQA_2026_CATEGORY_TIPS`` from the profile
module to build the unified string at import time (single source of
truth per D-009 — content lives in the profile; this solver merely
presents it without dispatching).
"""
...
# ---------------------------------------------------------------------------
# Unified category tips — concatenation of all 8 categories, computed once
# at import time. Per D-009 the content of source lives in the profile
# (single source of truth); we only present it differently here.
# ---------------------------------------------------------------------------
...
class RvlmUnifiedProgram:
    ...
    def solve_document(self, document: Document) -> tuple[dict[str, str], dict[str, list[dict]]]:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # UNIFIED-TIPS ABLATION: ignore document.doc_category; inject
            # the concatenation of all 8 category tip blocks instead of
            # dispatching per-document. See module docstring.
            tips = _UNIFIED_TIPS
...
def create_rvlm_unified_program(
