"""Code-only RLM — no reasoning field, model generates only Python code.

Saves tokens by eliminating the reasoning field. The model's chain-of-thought
(if any) happens via thinking tokens or code comments instead.

Based on lean_rlm.py with the reasoning output field removed.
"""
...
Use code comments for planning/reasoning if needed.
...
class CodeRLM(Module):
    """Code-only RLM — generates only Python code, no reasoning field."""
    ...
    def _build_signatures(self) -> tuple[Signature, Signature]:
        ...
        # Code-only: no reasoning field, just code
        action_sig = (
            ...
            .append("code", dspy.OutputField(desc="Python code to execute. Use comments for planning."), type_=str)
        )
    ...
    def _extract_fallback(
        ...
        logger.warning("CodeRLM reached max iterations, using extract to get final output")
    ...
    def _process_execution_result(
        ...
        if isinstance(result, str) and result.startswith("[Error]"):
            ...
            return history.append(reasoning="", code=code, output=output)
        ...
        if final_output is not None:
            ...
            final_history = history.append(
                reasoning="", code=code, output=f"FINAL: {parsed_outputs}"
            )
            return Prediction(
                **parsed_outputs,
    ...
    def _execute_iteration(
        ...
        try:
            ...
            logger.warning("CodeRLM iteration %d/%d: parse error: %s", iteration + 1, self.max_iterations, e)
        ...
        if self.verbose:
            logger.info(
                f"CodeRLM iteration {iteration + 1}/{self.max_iterations}\nCode:\n{action.code}"
            )
