"""Patched RLM — uses SubprocessInterpreter instead of DSPy's Deno/Pyodide.

Based on epiq's RLM v4 and DSPy's RLM, adapted for docvqa.
Uses a persistent CPython subprocess for tool execution, avoiding
Deno/Pyodide WASM limitations.
"""
...
4. MINIMIZE RETYPING (INPUTS & OUTPUTS) - When values are long, precise, or error-prone (IDs, numbers, code, quotes), re-access them via variables and parse/compute in code instead of retyping.
5. SUBMIT ONLY AFTER SEEING OUTPUTS - SUBMIT ends the current run immediately. If you need to inspect printed output, run it in one step, review the result, then call SUBMIT in a later step.

You have max {max_iterations} iterations. When done, call SUBMIT() with your output."""
...
class RLM(Module):
    """RLM using SubprocessInterpreter (CPython subprocess) for reliable tool execution."""
    ...
    def _process_execution_result(
        ...
        reasoning = getattr(pred, "reasoning", None) or ""
        code = _strip_code_fences(pred.code) if pred.code else ""

        # Handle HistoryReset signal from SubprocessInterpreter
        if isinstance(result, HistoryReset):
            return REPLHistory().append(
                reasoning="History reset by agent",
                code=code,
    ...
    def _execute_iteration(
        ...
        try:
            ...
            logger.warning("RLM iteration %d/%d: parse error after adapter retries: %s", iteration + 1, self.max_iterations, e)
