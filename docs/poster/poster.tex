...
reasoner-LM writes Python that calls a \emph{frozen} VLM (\texttt{batch\_look}) on
regions it chooses: it \keyval{surveys} pages, \keyval{locates} the table,
...
stronger (27B) VLM: accuracy climbs $\sim$8~pts at both reasoner sizes. Same brain,
better eyes --- a perception bottleneck.}

\vspace{5pt}
The lift is a \keyval{capacity gate}: the model must be a good-enough coder to drive
the REPL. A 31B Gemma clears its ReAct baseline by 14~pts; a 4B Gemma collapses ---
